...
avg_best_score_a = [np.mean(Best_Score_Attempts[i:i+3]) for i in range(0, len(Best_Score_Attempts), 3)]
avg_best_score = [np.mean(Best_Scores[i:i+3]) for i in range(0, len(Best_Scores), 3)]
...
fig2, ax2 = plt.subplots(4, 1, figsize=(10, 16))  # Updated to 4 plots
...
ax2[0].plot(avg_intentos, avg_reward_dist, linestyle='-', color='green', label='Reward Dist')
ax2[0].set_xlabel('Intento')
ax2[0].set_ylabel('Puntaje Distancia')
ax2[0].set_title('Puntaje Promedio Distancia por cada 100 intentos')
ax2[0].legend()

# Plot 4: Reward Control by Attempt
ax2[1].plot(avg_intentos, avg_reward_ctrl, linestyle='-', color='blue', label='Reward Ctrl')
ax2[1].set_xlabel('Intento')
ax2[1].set_ylabel('Puntaje Control')
ax2[1].set_title('Puntaje Promedio Control por cada 100 intentos')
ax2[1].legend()

# Plot 5: Reward Near Object by Attempt
ax2[2].plot(avg_intentos, avg_reward_near_object, linestyle='-', color='red', label='Reward Near Object')
ax2[2].set_xlabel('Intento')
ax2[2].set_ylabel('Recompensa Cerca del Objeto')
ax2[2].set_title('Recompensa Promedio por Acercarse al Objeto por cada 100 intentos')
ax2[2].legend()

# Plot 6: Reward Contact by Attempt
ax2[3].plot(avg_intentos, avg_reward_contact, linestyle='-', color='purple', label='Reward Contact')
ax2[3].set_xlabel('Intento')
ax2[3].set_ylabel('Recompensa Contacto')
ax2[3].set_title('Recompensa Promedio por Contacto Exitoso por cada 100 intentos')
ax2[3].legend()
